...
interface PlayerBarProps {
  onNext: () => void;
  onPrev: () => void;
  tracks: any[]; // Pass tracks array down to resolve metadata titles instantly
}
...
export const PlayerBar: React.FC<PlayerBarProps> = ({ onNext, onPrev, tracks = [] }) => {
  // 1. Consume the sandboxed audio engine states directly
  const { 
    isPlaying, 
    currentTrack, 
    currentTime, 
  ...
  // 2. Compute metadata reactively whenever the track path updates
  useEffect(() => {
    if (!currentTrack) {
    ...
    // Baseline fallback: slice the string path to pull raw file name if db match drops
  ...
  return (
    <div className="w-full h-20 bg-zinc-900 border-t border-zinc-800 px-6 flex items-center justify-between text-white select-none z-10">
      ...
      {/* Middle Section: Media Controls & Progress Timeline */}
